    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-800 rounded-lg p-4 border border-slate-600">
          ...
          <p className={`text-2xl font-bold ${totalSales >= 0 ? 'text-green-400' : 'text-red-400'}`}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-900 border-b border-slate-600">
                  ...
                  <th className="px-4 py-3 text-right text-slate-300 font-semibold">Sold For</th>
                  <th className="px-4 py-3 text-right text-slate-300 font-semibold">Profit</th>
                  ...
                  <tr key={sale.id} className="border-b border-slate-700 hover:bg-slate-700/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-400">
                      <span className={`px-2 py-1 rounded text-xs font-semibold ${
                        ...
                        {sale.item_type === 'deck' ? 'Deck' : 'Folder'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right text-slate-300">${sale.purchase_price.toFixed(2)}</td>
                    <td className="px-4 py-3 text-right text-slate-300">${sale.sell_price.toFixed(2)}</td>
