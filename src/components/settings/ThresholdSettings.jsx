  ...
  return (
    <div className="bg-gradient-to-br from-slate-800/90 to-slate-900/70 rounded-2xl p-6 space-y-6 border border-[var(--border)] shadow-xl">
      ...
      <div className="space-y-3">
        <p className="text-sm font-medium text-[var(--text-muted)]">Quick Presets</p>
          ...
          {Object.entries(QUICK_PRESETS).map(([key, preset]) => {
            ...
            return (
              ...
              >
                ...
                <span className="block text-[var(--text-muted)] mt-1">{preset.description}</span>
              </button>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-[var(--muted-surface)] rounded-xl p-4 border border-[var(--border)] hover:border-yellow-500/30 transition-colors">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-[var(--text-muted)]">2-50</span>
          </div>
          <input
            type="range"
            min="2"
        ...
        <div className="bg-[var(--muted-surface)] rounded-xl p-4 border border-[var(--border)] hover:border-green-500/30 transition-colors">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-[var(--text-muted)]">→ {thresholdSettings.baseStock * thresholdSettings.landMultiplier}</span>
          </div>
          <input
            type="range"
            min="2"
        ...
        <div className="bg-[var(--muted-surface)] rounded-xl p-4 border border-[var(--border)] hover:border-blue-500/30 transition-colors">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-[var(--text-muted)]">weeks</span>
          </div>
          <input
            type="range"
            min="1"
      ...
      <div className="bg-[var(--surface)] rounded-xl p-4 border border-[var(--border)]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-[var(--muted-surface)] rounded-lg p-3 border border-green-500/20">
            ...
            <p className="text-xs text-[var(--text-muted)]">{thresholdSettings.baseStock} × {thresholdSettings.landMultiplier}</p>
          </div>
          <div className="bg-[var(--muted-surface)] rounded-lg p-3 border border-blue-500/20">
            <p className="text-xs text-[var(--text-muted)] mb-1 flex items-center gap-1">
            ...
            <p className="text-xs text-[var(--text-muted)]">5/week × {thresholdSettings.velocityWeeks} weeks</p>
          </div>
          <div className="bg-[var(--muted-surface)] rounded-lg p-3 border border-purple-500/20">
            <p className="text-xs text-[var(--text-muted)] mb-1 flex items-center gap-1">
            ...
            <p className="text-xs text-[var(--text-muted)]">Under $0.50: base × 1.5</p>
          </div>
          <div className="bg-[var(--muted-surface)] rounded-lg p-3 border border-amber-500/20">
            <p className="text-xs text-[var(--text-muted)] mb-1 flex items-center gap-1">
            ...
            <p className="text-xs text-[var(--text-muted)]">Over $10: base × 0.3</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[var(--surface)] rounded-xl p-4 border border-[var(--border)]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
          ...
          <div className="bg-[var(--muted-surface)] rounded-lg p-2">
            ...
            <p className="text-xs text-[var(--text-muted)]">Basic Lands</p>
          </div>
          <div className="bg-[var(--muted-surface)] rounded-lg p-2">
            <p className="text-xl font-bold text-blue-400">{summaryStats.velocityBased}</p>
            <p className="text-xs text-[var(--text-muted)]">Velocity-Based</p>
          </div>
          <div className="bg-[var(--muted-surface)] rounded-lg p-2">
            <p className="text-xl font-bold text-purple-400">{summaryStats.priceBased}</p>
            <p className="text-xs text-[var(--text-muted)]">Price-Based</p>
          </div>
        </div>
      </div>
      ...
      <details className="bg-[var(--surface)] rounded-xl border border-[var(--border)] overflow-hidden">
        <summary className="p-4 cursor-pointer hover:bg-[var(--muted-surface)] transition-colors flex items-center gap-2 text-sm font-semibold text-[var(--text-muted)]">
        ...
        <div className="p-4 pt-0 max-h-64 overflow-y-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {inventory?.slice(0, 20).map(item => {
                ...
                return (
                  <tr key={item.id} className="border-b border-[var(--border)] hover:bg-[var(--muted-surface)]">
                    <td className="py-2 text-[var(--text-muted)] font-medium">{item.name}</td>
                    <td className="py-2 text-center text-[var(--text-muted)]">{item.low_inventory_threshold || 0}</td>
                    <td className="py-2 text-center text-yellow-400 font-semibold">{suggestion?.suggested || '-'}</td>
                    <td className="py-2 text-[var(--text-muted)]">{suggestion?.reason || '-'}</td>
                  </tr>
                );
              })}
            </tbody>
          ...
          {inventory && inventory.length > 20 && (
            <p className="text-xs text-[var(--text-muted)] text-center mt-3 py-2 border-t border-[var(--border)]">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <button
          ...
          className="px-4 py-3 bg-[var(--muted-surface)] hover:bg-slate-600/70 border border-[var(--border)] hover:border-slate-500 text-[var(--text-muted)] font-medium rounded-xl transition-all duration-200 flex items-center justify-center gap-2"
        >
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="text-sm text-[var(--text-muted)] space-y-1">
              ...
              <li>• <strong className="text-[var(--text-muted)]">Budget Cards</strong> (under $0.50) get 1.5× base stock for bulk inventory</li>
              <li>• <strong className="text-[var(--text-muted)]">Premium Cards</strong> (over $10) use 0.3× base stock to minimize capital in expensive singles</li>
            </ul>
          </div>
        </div>
      </div>
