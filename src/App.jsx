                        ...
                        <div className="mt-4 pt-4 border-t border-slate-700 hover:border-teal-500">
                          ...
                          containerItems[container.id].length > 0 ? (
                            <div className="space-y-2 max-h-96 overflow-y-auto">
                              {(() => {
                                // Group items by name+set
                                const grouped = {};
                                containerItems[container.id].forEach((item, idx) => {
                                  const key = `${item.name}|${item.set}`;
                                  if (!grouped[key]) {
                                    grouped[key] = {
                                      name: item.name,
                                  ...
                                  grouped[key].copies.push({ ...item, copyIdx: grouped[key].copies.length });
                                });
                                    ...
                                    <div key={groupKey} className="bg-slate-800 bg-opacity-50 border border-slate-600 rounded">
                                      <button
                                        onClick={() => setExpandedCardCopies(prev => ({
                                          ...prev,
                                          [`${container.id}-${groupKey}`]: !isExpanded
                                        }))}
                                            ...
                                            <div key={copyIdx} className="bg-slate-800 p-2 rounded border border-slate-600 text-xs space-y-2">
                                              ...
                                              <div className="grid grid-cols-2 gap-2">
                                                <div className="bg-slate-700 p-2 rounded">
                                                  ...
                                                  <DecklistCardPrice name={copy.name} set={copy.set} />
                                                </div>
                                                <div className="bg-slate-700 p-2 rounded">
